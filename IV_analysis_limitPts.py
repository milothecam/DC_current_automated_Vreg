...
# Index range for analysis (0-based, inclusive on both ends).
# Set FILE_IDX_START=0 and FILE_IDX_END=None to use all files.
FILE_IDX_START = 7
FILE_IDX_END   = 12   # e.g. 30 means files [0 .. 30] (31 files total) / None = use last.
...
# Apply index range slicing
idx_end = FILE_IDX_END + 1 if FILE_IDX_END is not None else None   # make end inclusive
filenames   = filenames[FILE_IDX_START:idx_end]
currents_mA = currents_mA[FILE_IDX_START:idx_end]
print(f"Using file indices [{FILE_IDX_START} .. {FILE_IDX_END if FILE_IDX_END is not None else len(Vdc_list)-1}]  ({len(filenames)} files)")
...
def plot_s2p_phase_vs_current(filenames, currents, target_freq, sparam="s12",
    ...
    Saves the figure automatically (if SAVE_PLOTS is True).
    """
    ...
    if SAVE_PLOTS:
        ...
        print("Plot saving skipped (SAVE_PLOTS = False)")
